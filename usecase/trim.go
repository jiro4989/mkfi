...
func Trim(rect domain.Rectangle, outDir string, targetFiles []string) ([]string, error) {
...
func trimImageFile(wg *sync.WaitGroup, q chan string, rect domain.Rectangle, outDir string, createdFiles []string, errs []error) {
	...
	for {
		...
		cImg, err := infrastructure.CropImage(src, infrastructure.CropConfig{
			Width:  w,
			Height: h,
			Anchor: image.Pt(x, y),
			Mode:   infrastructure.ImageTopLeft,
		})
		if err != nil {
			errs = append(errs, err)
			continue
